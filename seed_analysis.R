...
mod2 = aov(transformed_amount ~ term, data = stat_table)
summary(mod1)
summary.lm(mod2)
p_values_1 = coef(summary.lm(mod2))[-1,4]
p.adjust(p_values_1, method = "hochberg", n = length(p_values_1))
...
vocab_table = vocab_table[vocab_table$doc_counts > 5 & vocab_table$doc_counts < (nrow(d) * 0.25), ]
...
for(i in 1:length(terms)){
  ...
  p_value = t.test(y1$transformed_amount, y2$transformed_amount, alternative = "two.sided")
